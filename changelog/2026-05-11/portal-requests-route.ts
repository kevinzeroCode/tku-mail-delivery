...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!EMAIL_RE.test(userEmail)) {
      return NextResponse.json({ error: 'userEmail 格式無效' }, { status: 400 })
    }
    ...
    // IDOR guard: only the exact registered recipient may submit requests.
    // Mails without a registered email are blocked until an admin assigns one.
    // Once O365 MSAL is integrated, replace `userEmail` with the verified JWT claim.
    if (!item.recipientEmail || item.recipientEmail !== userEmail) {
      return NextResponse.json({ error: '無權限操作此郵件' }, { status: 403 })
    }
