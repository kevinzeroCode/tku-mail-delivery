...
import {
  ...
  Divider, Upload, Image, Collapse, Button, message,
} from 'antd'
    ...
    >
      {item && (
        <>
          ...
          <Space align="start" style={{ marginBottom: 8 }} wrap>
            <Upload
              accept="image/*"
            ...
            {!showCamera ? (
              <Button size="small" icon={<CameraOutlined />} onClick={() => setShowCamera(true)}>
                直接拍照
              </Button>
            ) : (
              <Button size="small" icon={<CloseOutlined />} onClick={() => setShowCamera(false)}>
                關閉相機
              </Button>
            )}
            {photoPreview && (
