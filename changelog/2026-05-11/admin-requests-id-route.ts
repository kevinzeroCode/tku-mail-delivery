...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Wrap the entire approve/reject in a transaction to prevent double-processing
    // when two admins act on the same request simultaneously.
    const updated = await prisma.$transaction(async (tx) => {
      // Re-fetch inside transaction to guard against race condition
      const fresh = await tx.mailRequest.findUnique({ where: { id } })
      if (!fresh || fresh.status !== '待處理') {
      ...
      if (action === 'approve') {
        if (request.type === 'reject_return') {
          await tx.mailItem.update({
            ...
            data: { status: '已退回', returnDate: now },
          })
