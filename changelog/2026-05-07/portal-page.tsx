...
interface PortalMailItem {
  ...
  notes: string | null
  requests: { id: number; type: string; status: string; adminNote: string | null; createdAt: string }[]
}
...
function MailsTab({ email }: { email: string }) {
  ...
  const columns: ColumnsType<PortalMailItem> = [
    ...
    },
    {
      ...
      width: 160,
      render: (_, r) => {
        if (!r.requests.length) return <span style={{ color: '#bbb' }}>—</span>
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {r.requests.map(req => {
              const color = req.status === '待處理' ? 'gold' : req.status === '已核准' ? 'green' : 'default'
              const icon  = req.status === '已核准' ? '✓' : req.status === '已拒絕' ? '✗' : '⏳'
              return (
                <div key={req.id}>
                  <Tag color={color} style={{ marginRight: 0 }}>
                    {icon} {req.type === 'reject_return' ? '拒收'
                      : req.type === 'change_pickup' ? '異動方式'
                      : req.type === 'wrong_recipient' ? '更正收件人'
                      : '領取簽名'} {req.status}
                  </Tag>
                  {req.adminNote && (
                    <div style={{ fontSize: 11, color: '#555', marginTop: 2, paddingLeft: 2 }}>
                      收發室：{req.adminNote}
                    </div>
                  )}
                </div>
