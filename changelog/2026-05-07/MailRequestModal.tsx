...
export default function MailRequestModal({ item, requestType, userEmail, onSaved, onCancel }: Props) {
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      } else if (requestType === 'reject_return') {
        const values = form.getFieldsValue()
        requestData = { note: values.note ?? null }
      } else if (requestType === 'change_pickup') {
        const values = await form.validateFields()
        requestData = { newMethod: values.newMethod, note: values.note ?? null }
        ...
        <>
          ...
          {requestType === 'reject_return' && (
            <Form form={form} layout="vertical">
              <p style={{ marginTop: 0 }}>確認申請拒收此郵件？送出後，收發室將安排退回給寄件人。</p>
              <Form.Item name="note" label="留言給收發室（選填）">
                <Input.TextArea rows={2} placeholder="例如：非本人郵件、已搬遷…" />
              </Form.Item>
            </Form>
          )}
          ...
          {requestType === 'change_pickup' && (
            <Form form={form} layout="vertical">
              ...
              <Form.Item noStyle shouldUpdate={(prev, cur) => prev.newMethod !== cur.newMethod}>
                {({ getFieldValue }) => getFieldValue('newMethod') === '其他' && (
                  <Form.Item name="note" label="請說明"
                    rules={[{ required: true, message: '選擇「其他」請填寫說明' }]}>
                    <Input.TextArea rows={2} placeholder="請說明希望如何處理" />
                  </Form.Item>
                )}
              </Form.Item>
            </Form>
